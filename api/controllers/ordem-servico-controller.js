...
exports.postOrdemServico = (req, res, next)=>{
    mysql.getConnection((error, conn) => {
        if(error){return res.status(500).send({error: error})}
        conn.query(
            `insert into ordem_servico (data_abertura, previsao_entrega, files, id_veiculo, id_funcionario, id_servico) values (?, ?, ?, ?, ?, ?);`,
            [req.body.data_abertura ,req.body.previsao_entrega, req.body.files, req.body.id_veiculo, req.body.id_funcionario, req.body.id_servico],
            (error, resultado, fields) => {
                if(error){return res.status(500).send({error: error})}
                res.status(201).send({
